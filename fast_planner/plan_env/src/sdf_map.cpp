...
void SDFMap::initMap(ros::NodeHandle& nh) {
	...
	map_obj_pub_ = node_.advertise<sensor_msgs::PointCloud2>("/object_at_robot_height", 10);
...
void SDFMap::updateESDF3d() {
	...
	for (int x = min_esdf[0]; x <= max_esdf[0]; x++) {
		for (int y = min_esdf[1]; y <= max_esdf[1]; y++) {
			fillESDF(
				[&](int z) {
					return md_.occupancy_buffer_inflate_[toAddress(x, y, z)] == 1 ? 0
	...
	for (int x = min_esdf[0]; x <= max_esdf[0]; x++) {
		for (int z = min_esdf[2]; z <= max_esdf[2]; z++) {
			fillESDF([&](int y) { return md_.tmp_buffer1_[toAddress(x, y, z)]; },
					 [&](int y, double val) { md_.tmp_buffer2_[toAddress(x, y, z)] = val; }, min_esdf[1], max_esdf[1],
					 1);
		}
	}
...
void SDFMap::raycastProcess() {
	...
	while (!md_.cache_voxel_.empty()) {
		...
		md_.occupancy_buffer_[idx_ctns] = std::min(
			std::max(md_.occupancy_buffer_[idx_ctns] + log_odds_update, mp_.clamp_min_log_), mp_.clamp_max_log_);
	}
}
...
void SDFMap::clearAndInflateLocalMap() {
	...
	Eigen::Vector3i min_cut =
		md_.local_bound_min_ - Eigen::Vector3i(mp_.local_map_margin_, mp_.local_map_margin_, mp_.local_map_margin_);
	Eigen::Vector3i max_cut =
		md_.local_bound_max_ + Eigen::Vector3i(mp_.local_map_margin_, mp_.local_map_margin_, mp_.local_map_margin_);
	boundIndex(min_cut);
	boundIndex(max_cut);
...
void SDFMap::updateOccupancyCallback(const ros::TimerEvent& /*event*/) {
	...
	if (mp_.show_occ_time_)
		ROS_WARN("Fusion: cur t = %lf, avg t = %lf, max t = %lf", (t2 - t1).toSec(), md_.fuse_time_ / md_.update_num_,
				 md_.max_fuse_time_);
...
void SDFMap::updateESDFCallback(const ros::TimerEvent& /*event*/) {
	...
	if (mp_.show_esdf_time_)
		ROS_WARN("ESDF: cur t = %lf, avg t = %lf, max t = %lf", (t2 - t1).toSec(), md_.esdf_time_ / md_.update_num_,
				 md_.max_esdf_time_);
...
void SDFMap::depthPoseCallback(const sensor_msgs::ImageConstPtr& img, const geometry_msgs::PoseStampedConstPtr& pose) {
	...
	md_.camera_q_ = Eigen::Quaterniond(pose->pose.orientation.w, pose->pose.orientation.x, pose->pose.orientation.y,
...
void SDFMap::publishMapInflate(bool all_info) {
	pcl::PointXYZ pt;
	pcl::PointCloud<pcl::PointXYZ> cloud;
	pcl::PointCloud<pcl::PointXYZ> robot_height_obj;
	...
	for (int x = min_cut(0); x <= max_cut(0); ++x)
		for (int y = min_cut(1); y <= max_cut(1); ++y)
			for (int z = min_cut(2); z <= max_cut(2); ++z) {
				...
				pt.z = pos(2);
				cloud.push_back(pt);
				if (fabs(pos(2) - robot_height) < 0.1 && pos(0) >= 0.0 && pos(1) >= 0.0) {
					obj.polygon.points[0].x = pos(0);
					obj.polygon.points[0].y = pos(1);
					obj.polygon.points[0].z = 0.0;
					global_map.obstacles_arr.obstacles.push_back(obj);
					robot_height_obj.push_back(pt);
					ROS_INFO("It should have size");
				}
			}
	...
	robot_height_obj.width = robot_height_obj.points.size();
	robot_height_obj.height = 1;
	robot_height_obj.is_dense = true;
	robot_height_obj.header.frame_id = mp_.frame_id_;


  global_map.obstacles_arr.header.stamp = ros::Time::now();//s
	global_map.obstacles_arr.header.frame_id = mp_.frame_id_;
  ...
  global_map.obstacles_arr.obstacles.clear();
	...
	pcl::toROSMsg(robot_height_obj, cloud_msg2);
	map_obj_pub_.publish(cloud_msg2);  //
